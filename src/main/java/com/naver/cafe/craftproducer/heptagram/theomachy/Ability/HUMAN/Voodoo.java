...
public class Voodoo extends Ability {
    ...
    public void description() {
        ...
        player.sendMessage(
                ...
                + " ===================");
        player.sendMessage(
                ChatColor.YELLOW + "[ 부두술사 ]  " + ChatColor.RED
                + "[ 인간 ]  " + ChatColor.BLUE + "Active  " + ChatColor.GREEN
                + "Rank[ A ]");
        player.sendMessage(
                "팻말을 이용해서 상대를 타격할 수 있는 능력입니다.\n"
                        ...
                        + ChatColor.GREEN + "펫말에 이름을 적을시 "
                        + ChatColor.WHITE + " 코블스톤 " + stack0
                        + "개 소모, 쿨타임 " + coolTime0 + "초");
    }
    ...
    public void T_Passive(SignChangeEvent event) {
        ...
        if (target != null) {
            ...
            player.sendMessage(
                    ChatColor.RED + targetName + ChatColor.WHITE
                    + " 를(을) 팻말과 연결시켰습니다.");
            target.sendMessage(
                    ChatColor.RED + "부두술사가 당신을 위협합니다.");
            Timer t = new Timer();
