...
public class Theomachy extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        getLogger().info("플러그인 활성화  " + PluginData.buildnumber + "  " + PluginData.version);
        ...
        getLogger().info("---------------------------------------");
        getLogger().info("게임 시작시 인벤토리 클리어 : " + String.valueOf(INVENTORY_CLEAR));
        getLogger().info("게임 시작시 스카이블럭 기본 아이템 지급 : " + String.valueOf(GIVE_ITEM));
        getLogger().info("게임 시작시 몬스터,동물,아이템삭제 : " + String.valueOf(ENTITIES_REMOVE));
        getLogger().info("리스폰시 침대 무시 : " + String.valueOf(IGNORE_BED));
        getLogger().info("서버 자동저장 : " + String.valueOf(AUTO_SAVE));
