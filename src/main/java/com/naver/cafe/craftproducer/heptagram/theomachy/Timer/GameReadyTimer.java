...
public class GameReadyTimer extends TimerTask {
    ...
    public GameReadyTimer() {
        ...
        setting[0] = Theomachy.INVENTORY_CLEAR
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[1] = Theomachy.GIVE_ITEM
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[2] = Theomachy.IGNORE_BED
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[3] = Theomachy.AUTO_SAVE
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[4] = Theomachy.ANIMAL
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[5] = Theomachy.MONSTER
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        setting[7] = Theomachy.ENTITIES_REMOVE
                ? ChatColor.AQUA + "ON"
                : ChatColor.RED + "OFF";
        difficulty = null;
        ...
        if (GameHandler.Ready && count < 45) {
            ...
            case 4:
                Bukkit.broadcastMessage(
                        ChatColor.RED
                                + "신들의 전쟁 플러그인은 스카이블럭 전용이며 야생,하드코어로는 부적합합니다");
                Bukkit.broadcastMessage(
                        ChatColor.RED
                                + "이점을 숙지 하시고 게임을 즐기시길 바랍니다.");
                break;

            case 8:
                Bukkit.broadcastMessage(
                        ChatColor.AQUA + "플러그인 배포중인곳");
                Bukkit.broadcastMessage(
                        ChatColor.GREEN + "http://cafe.naver.com/craftproducer");
                Bukkit.broadcastMessage(
                        ChatColor.WHITE
                                + "채팅창 여시고 마우스 클릭하시면 해당까페로 이동합니다.");
                break;

            case 12:
                Bukkit.broadcastMessage(
                        ChatColor.GREEN + "****** 서버 세팅상태 ******");
                Bukkit.broadcastMessage(
                        ChatColor.WHITE
                                + "게임 시작시 인벤토리 클리어 : "
                                + setting[0]);
                ...
                Bukkit.broadcastMessage(
                        ChatColor.WHITE
                                ...
                                + setting[7]);
                Bukkit.broadcastMessage(
                        ChatColor.WHITE + "부활시 침대 무시 : "
                        + setting[2]);
                Bukkit.broadcastMessage(
                        ChatColor.WHITE + "오토 세이브 : " + setting[3]);
                Bukkit.broadcastMessage(
                        ChatColor.WHITE + "동물 스폰 : " + setting[4]);
                Bukkit.broadcastMessage(
                        ChatColor.WHITE + "몬스터 스폰 : " + setting[5]);
                Bukkit.broadcastMessage(
                        ChatColor.WHITE + "서버 난이도 : " + setting[6]);
                Bukkit.broadcastMessage(
                        ChatColor.GREEN + "***************************");
                break;
            ...
            case 20:
                Bukkit.broadcastMessage(
                        ChatColor.AQUA
                                + "현재 능력이 적용되지 않은 플레이어 목록");
                for (int i = 0; i < playerList.length; i++) {
                    if (!GameData.PlayerAbility.containsKey(
                            playerList[i].getName())) {
                        Bukkit.broadcastMessage(
                                i + 1 + ".  " + ChatColor.GOLD
                                + playerList[i].getName());
            ...
            case 29:
                Bukkit.broadcastMessage(
                        "제작자 : " + ChatColor.AQUA + "Septagram(칠각별)");
                Bukkit.broadcastMessage(
                        "제작자 블로그 : http://blog.naver.com/septagram/");
                Bukkit.broadcastMessage(
                        "버그 신고 사항은 " + ChatColor.DARK_GREEN
                        + "septagram@naver.com " + ChatColor.WHITE
                        + "으로 로그를 보내주세요");
                break;
            ...
            case 34:
                Bukkit.broadcastMessage(
                        ChatColor.AQUA
                                + "잠시 후 능력이 활성화 되며 팀 스폰지역으로 텔레포트 됩니다");
                break;

            case 35: 
                Bukkit.broadcastMessage(ChatColor.RED + "5 초전");
                break;

            case 36:
                Bukkit.broadcastMessage(ChatColor.RED + "4 초전");
                break;

            case 37:
                Bukkit.broadcastMessage(ChatColor.RED + "3 초전");
                break;

            case 38:
                Bukkit.broadcastMessage(ChatColor.RED + "2 초전");
                break;

            case 39:
                Bukkit.broadcastMessage(ChatColor.RED + "1 초전");
                break;
                ...
                for (Player e : playerList) {
                    ...
                    if (teamName != null) {
                        ...
                        if (location != null) {
                            ...
                            e.sendMessage(
                                    ChatColor.RED
                                            + "팀의 스폰지역이 설정되지 않아 기본 스폰지역으로 이동합니다.");
                            e.teleport(spawnLocation);
                        }
                    } else {
                        e.sendMessage(
                                ChatColor.RED
                                        + "팀이 지정되지 않아 기본 스폰지역으로 이동합니다.");
                        e.teleport(spawnLocation);
                    }
                }
                break;
