...
public class Teleporter extends Ability {
    ...
    public void description() {
        ...
        player.sendMessage(
                ...
                + " ===================");
        player.sendMessage(
                ChatColor.YELLOW + "[ 텔레포터 ]  " + ChatColor.RED
        ...
        if (CoolTimeChecker.Check(player, 1)
            ...
            if (BlockFilter.AirToFar(player, block)) {
                ...
                if ((block0.getTypeId() == 0 || block1.getTypeId() == 78)
                    ...
                    player.sendMessage(
                            "텔레포트 할 수 있는 공간이 없어 텔레포트에 실패 했습니다.");
                }
            }
        }
    }
        ...
        if (CoolTimeChecker.Check(player, 2)
            ...
            if (abilitytarget != null) {
                ...
                if (target != null) {
                    ...
                    target.sendMessage(
                            "텔레포터의 능력에 의해 위치가 텔레포터의 위치로 변경되었습니다.");
                } else {
                    player.sendMessage(
                            "플레이어가 온라인이 아닙니다.");
                }
                ...
                player.sendMessage(
                        "타겟이 지정되지 않았습니다. (타겟 등록법 : /x <Player>)");
            }
        }
    }
    ...
    public void targetSet(CommandSender sender, String targetName) {
        ...
        if (playerTeamName != null && targetTeamName != null
                && playerTeamName.equals(targetTeamName)) {
            if (!playerName.equals(targetName)) {
                this.abilitytarget = targetName;
                sender.sendMessage(
                        "타겟을 등록했습니다.   " + ChatColor.GREEN
                        + targetName);
            } else {
                sender.sendMessage(
                        "자기 자신을 타겟으로 등록 할 수 없습니다.");
            }
        } else {
