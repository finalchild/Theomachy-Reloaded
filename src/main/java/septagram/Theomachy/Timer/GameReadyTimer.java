	...
	{		
		if (GameHandler.Ready && count<45)
		{
			...
			case 4:
				...
				break;
			case 8:
				Bukkit.broadcastMessage(ChatColor.AQUA+"플러그인 배포중인곳");
				Bukkit.broadcastMessage(ChatColor.GREEN+"http://cafe.naver.com/craftproducer");
				Bukkit.broadcastMessage(ChatColor.WHITE+"채팅창 여시고 마우스 클릭하시면 해당까페로 이동합니다.");
				break;
			case 12:
				Bukkit.broadcastMessage(ChatColor.GREEN+"****** 서버 세팅상태 ******");
				Bukkit.broadcastMessage(ChatColor.WHITE+"게임 시작시 인벤토리 클리어 : "+setting[0]);
				Bukkit.broadcastMessage(ChatColor.WHITE+"게임 시작시 기본 아이템 지급 : "+setting[1]);
				Bukkit.broadcastMessage(ChatColor.WHITE+"게임 시작시 몬스터,동물,아이템삭제 : "+setting[7]);
				Bukkit.broadcastMessage(ChatColor.WHITE+"부활시 침대 무시 : "+setting[2]);
